...
WHAT IS AND IS NOT HERE. There is no image in the simulated
world. A `depiction` artifact records that a surface was
exposed; it has no content, because the world has no scene --
no geometry, no light, nothing to photograph. Rendering a
picture of one would be inventing pixels, which is the same
error as inventing a name or an answer.

What IS real is the ledger. Forty bands, twelve thousand years,
and every craft each of them held at every step. That is a
genuine two-dimensional array of measured values, and encoding
it produces a genuine JPEG file that a genuine decoder opens.

So this is OUR rendering of THEIR data, and it is labelled that
way. The compression is the derived part: the quantization table
below comes from the contrast sensitivity of the eye, not from
Annex K of the specification.
